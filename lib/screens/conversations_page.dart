...
import 'package:adote_um_pet/models/message_entity.dart';
...
class _ConversationsPageState extends State<ConversationsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: _messagesAndUsersMap.isEmpty
          ? const Center(
        child: EmptyStatePane(
            description: "Não há mensagens para exibir",),
      )
          : ListView.builder(
        itemCount: _messagesAndUsersMap.length,
        itemBuilder: (context, index) {
          Message message = _messagesAndUsersMap.keys.elementAt(index);
          User senderUser = _messagesAndUsersMap.values
              .elementAt(index)
              .keys
              .elementAt(0);
          User recipientUser = _messagesAndUsersMap.values
              .elementAt(index)
              .values
              .elementAt(0);
